...
const PIPELINE_STEPS = [
  {
    ...
    description: "Leer ventanas de tráfico desde el pipeline local.",
    icon: Database,
    tone: "cyan",
  },
  {
...
const CONSOLE_MESSAGES = [
  "establishing uplink with FastAPI service",
  "requesting local pipeline execution",
  "loading KMeans artifact",
  "extracting traffic windows",
  "aligning feature columns",
  "running label assignment",
  "serializing tactical dataset",
  "writing /public/data/predictions.json",
    ...
    <div className="grid gap-5 lg:grid-cols-[0.92fr_1fr]">
      <section className="overflow-hidden rounded-md border border-cyan-200/20 bg-cyan-200/10 p-5 shadow-2xl shadow-cyan-950/20">
        ...
        <p className="mt-3 text-sm leading-7 text-white/60">
          Conecta el dashboard con el servicio FastAPI local, ejecuta el pipeline de inferencia y prepara el archivo que alimenta el espacio 3D de Log Hound.
        </p>
        ...
        <div className="mt-5 grid gap-3 rounded-md border border-white/10 bg-black/24 p-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs text-white/58">
            <div className="rounded-md border border-white/10 bg-white/[0.04] p-3">
              ...
              <div className="mt-1 font-mono text-cyan-100">POST /api/predictions</div>
            </div>

            <div className="rounded-md border border-white/10 bg-white/[0.04] p-3">
              <div className="text-white/38">Output</div>
              <div className="mt-1 font-mono text-emerald-100">/data/predictions.json</div>
            </div>
          </div>
        </div>
        ...
        <p className="mt-3 text-xs leading-5 text-white/38">
          Esto ejecuta el pipeline local y sobrescribe el dataset operativo del dashboard. Para producción, después conviene mover el output a Supabase Storage o DB.
        </p>
      </section>
    ...
    <section className="overflow-hidden rounded-md border border-cyan-200/20 bg-black/48 shadow-2xl shadow-cyan-950/30 backdrop-blur-xl">
      ...
      <div className="relative min-h-[420px] p-4 font-mono text-xs">
        ...
        <div className="relative">
          ...
          <ConsoleLine>session: local-fastapi-uplink</ConsoleLine>
          <ConsoleLine>mode: inference / tactical export</ConsoleLine>
  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[0.9fr_1fr]">
      <div className="overflow-hidden rounded-md border border-emerald-300/30 bg-emerald-300/10 p-5 shadow-2xl shadow-emerald-950/20">
        ...
        <p className="mt-3 text-sm leading-7 text-white/60">
          El pipeline respondió correctamente y Next almacenó el dataset operativo en el directorio público.
        </p>
        ...
        <div className="mt-5 grid gap-2 rounded-md border border-white/10 bg-black/24 p-4 font-mono text-xs text-white/64">
          ...
          <div className="flex justify-between gap-3">
            <span>output</span>
            <span className="text-cyan-100">{result?.output ?? "/data/predictions.json"}</span>
          </div>
          <div className="flex justify-between gap-3">
            <span>status</span>
  ...
  return (
    <section className="overflow-hidden rounded-md border border-red-300/25 bg-red-300/10 p-5 shadow-2xl shadow-red-950/20">
      ...
      <div className="mt-5 rounded-md border border-white/10 bg-black/30 p-4 font-mono text-xs text-white/58">
        ...
        <ConsoleLine>check: FastAPI online at 127.0.0.1:8000</ConsoleLine>
        <ConsoleLine>check: FASTAPI_URL in .env.local</ConsoleLine>
        <ConsoleLine>check: /api/predictions route active</ConsoleLine>
      </div>
  ...
  async function startGeneration() {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `La API no devolvió JSON. Status: ${response.status}. Respuesta: ${rawText.slice(0, 220)}`,
        );
      }

      if (!response.ok || payload.status !== "ok") {
        throw new Error(payload.message || payload.detail || "No se pudo generar la data.");
      }
      ...
      setConsoleLines((lines) => [
        ...
        `records indexed: ${payload.records ?? "N/A"}`,
        `output committed: ${payload.output ?? "/data/predictions.json"}`,
        "dataset online",
      ]);
      ...
      setConsoleLines((lines) => [...lines, "pipeline response failed"]);
